...
describe("listAuditEvents", () => {
  it("asks for the whole journal when nothing is filtered", async () => {
    ...
    expect(request).toHaveBeenCalledWith("/api/panel/audit-events");
  });

  it("covers the whole of the last day, not its first second", async () => {
    ...
    expect(request).toHaveBeenCalledWith(
      "/api/panel/audit-events?since=2026-09-01T00%3A00%3A00&until=2026-09-05T23%3A59%3A59",
    );
  });
