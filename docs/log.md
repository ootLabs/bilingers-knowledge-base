...
**Done:** `panel_users` with roles, sessions, a login audit and one-time password tokens (migration 0002); `/api/panel` endpoints for login, logout, own password, and administrator-only account management; `python -m app.cli create-admin` for the first account; 154 backend tests at 100% coverage.
