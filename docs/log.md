...
**Done:** `POST /chat` accepts `{question, session_token}`, upserts a `ChatSession` by token, writes and commits a `Query` row, then streams a fixed placeholder answer chunk by chunk (`text/plain`). No retrieval, model call, or orchestration yet. `backend/app/services/`, `backend/app/schemas/` created with their first files. 66 backend tests at 100% coverage.
**Decisions:** `get_or_create_chat_session` only flushes a new row; `record_query`'s commit makes both it and the query durable together, so a failure between the two never leaves an orphaned session. A lost race between two first-time requests on the same brand-new `session_token` is recovered by catching the unique-constraint `IntegrityError` and re-reading the row, instead of surfacing as a 503. `answer` stays NULL throughout: no model runs, and the `queries_answer_requires_kb_version` constraint would reject a non-null answer with no knowledge base version anyway. Service failures raise `ChatServiceUnavailable`, a domain exception, rather than letting `SQLAlchemyError` cross into the router, per `docs/conventions.md`. `PLACEHOLDER_ANSWER` is English, not Polish, because `docs/llm/i18n.md` bans hardcoded Polish in the backend outright. Moved `db_session`/`migrated_database` test fixtures from `test_models.py` into `conftest.py` so `test_chat.py` could reuse them.
**Watch out:** `ChatRequest` relies on `ConfigDict(str_strip_whitespace=True)` so stripping happens before `max_length` is checked; a hand-rolled `field_validator` would run after Field-level constraints and reject valid, merely padded input.
