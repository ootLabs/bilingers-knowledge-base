...
- **Errors:** raise `HTTPException` at the router boundary; services raise domain exceptions and let the router translate them. Never return a `200` with an error body. A domain exception that can also surface from a dependency, before any handler body runs, is translated once by an app-level exception handler instead (`PanelServiceUnavailable` in `main.py`) - the router cannot catch what it never entered.
