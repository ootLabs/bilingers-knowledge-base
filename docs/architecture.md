...
| 2026-08-28 | Five failed logins lock the account for fifteen minutes; every attempt is recorded, including for addresses that match no account | An attack on a five-account panel looks like repeated failures against addresses that do not exist, and that pattern is invisible if only real accounts are logged. The counter lives on the account, not in the audit table, so pruning the audit cannot quietly disable the limit. Known cost, accepted for now: the lock is per account and nothing limits by address, so anyone can keep all editors out for five requests per account per fifteen minutes, and a `423` next to a `401` confirms which addresses have accounts. Availability, not access (an administrator-issued reset clears the lock), and `panel_login_attempts.ip_address` is already being written for whatever adds a per-address limit |
