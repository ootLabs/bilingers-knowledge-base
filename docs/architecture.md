...
| 2026-09-03 | Every panel service function reachable from HTTP carries `unavailable_on_database_failure`, and one app-level handler answers `PanelServiceUnavailable` with `503` | Applies the 2026-08-26 chat precedent to the panel, which had been leaking `SQLAlchemyError` and answering a dropped connection with a 500: the status that tells a client its request was hopeless, on the one condition where retrying works, with a traceback in an unauthenticated response. A decorator rather than chat's explicit blocks because the rule is uniform here (chat has to tell a `DataError` on bad input apart from an outage; the panel validates every string at the boundary instead, so there is no client-fault branch), and one handler in `main.py` rather than nine `except` clauses because `resolve_session` fails inside the `current_panel_session` dependency, where no handler body exists to catch anything |
| 2026-09-03 | The panel's address pattern excludes control characters, and the per-account lockout is not charged against a deactivated account | Two more findings from the same review. A NUL byte passed `[^@\s]` and `strip()`, so the driver refused it instead of the boundary: an unauthenticated 500 with no audit row, where the input was the client's to fix (422). And counting wrong passwords against a switched-off account let anyone lock an account nothing can unlock, since reactivation does not clear `locked_until` - the owner would come back to a 401 on their own correct password, which is the outcome the correct-password branch was already written to avoid. The attempt is still recorded with its real reason, so the audit trail still shows the guessing |
