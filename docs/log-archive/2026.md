...
Entries moved out of `../log.md` when it hit its 20 entry cap. Newest first, same format. Nothing here is loaded into a session by default; read it only when chasing something the current log no longer covers.
