...
| `frontend/app/chat/ChatPanel.tsx` | Client component owning the four T-63 states (empty, waiting, answering, failed): question box, submit, retry, and the streamed answer. Resolves the translation keys the backend streams into copy, drops any it cannot resolve, and fails rather than showing a blank answer if none of them resolve. The conversation proper (history, follow-up threads, T-61's entry screen) is T-62, not here |
...
| `frontend/lib/api-client.ts` | The only place the browser calls the backend: `API_URL`, `streamAnswer` (POST `/chat`, yields the stream), `createSessionToken`, `ChatRequestError` and the `ChatFailure` vocabulary that HTTP statuses collapse into. Never reads a failed response's body |
...
| `frontend/lib/api-client.test.ts` | The posted field names, chunk order, a multi-byte character split across two chunks, every status-to-failure mapping, an unmapped status degrading to `unreachable`, an abort rethrown as-is, and that a failed response's body is never read |
