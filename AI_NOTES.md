...
- **Verified:** `docker compose up --build` starts all three containers; `/health` and `/health/db` return ok, the frontend serves 200, and `db/init/01_schema.sql` created `health_probe`.
- **Watch out:**
  - `frontend/package-lock.json` does not exist, so the image uses `npm install`, not `npm ci` — builds are not reproducible until a lockfile is committed.
